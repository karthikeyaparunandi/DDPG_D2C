...
#    b,a=signal.butter(8,0.025)
#    plt.plot(x,y/y[-1],color='g')
#    plt.plot(x,signal.filtfilt(b,a,y/y[-1]),alpha=0.8,color='r',linewidth=3)
    
    plt.xlabel('time/s',fontsize=16)
    #plt.xlabel('episodes',fontsize=16)
    plt.ylabel('cost / cost_norm',fontsize=16)
    plt.show()
    print("NUM = {value1}".format(value1=y.shape[0]))
...
def perfcheck(nstart=0,nend=100,type='error',noisemax=100):
    if type=='cost':
        ...
        plt.xlabel('Std dev of perturbed noise (Percent of max. control)',fontsize=16)
        plt.ylabel('cost per step',fontsize=16)
        plt.show()  
        print('averaged by {value1} rollouts'.format(value1=y.shape[1]))
    ...
    if type=='error':
        ...
        plt.xlabel('Std dev of perturbed noise (Percent of max. control)',fontsize=16)
        plt.ylabel('L2-norm of terminal state error',fontsize=16)
        plt.show()  
        print('averaged by {value1} rollouts'.format(value1=y.shape[1]))
        
def errcompare(nstart=0,nend=100, pointnum=10, testnum=200, by='state'):
    # pointnum: total number of noise level checked
