...
def build_callbacks(env_name):
    checkpoint_weights_filename = 'results/Cart2Pole/ddpg_' + env_name + '_weights_{step}.h5f'
    log_filename = 'results/Cart3Pole/exp_1/ddpg_{}_log.json'.format(env_name)
...
random_process = OrnsteinUhlenbeckProcess(size=nb_actions, theta=.15, dt=0.1, mu=0., sigma=.3)
...
# Ctrl + C.
agent.fit(env, nb_steps=500000, visualize=True, callbacks=callbacks, verbose=1, gamma=GAMMA, nb_max_episode_steps=60)

# After training is done, we save the final weights.
agent.save_weights('results/Cart3Pole/exp_1/ddpg_{}_weights.h5f'.format(ENV_NAME), overwrite=True)
#agent.load_weights('results/InvertedPendulum/exp_6/ddpg_{}_weights.h5f'.format(ENV_NAME))

# Finally, evaluate our algorithm for 5 episodes.
history, state_history_nominal, episode_reward_nominal = agent.test(env, nb_episodes=1, visualize=True, action_repetition=1, callbacks=test_callbacks, nb_max_episode_steps=60, \
                                                         initial_state=[0, np.pi, 0, 0, 0, 0], std_dev_noise=0, gamma=GAMMA)
u_max = 12
print(episode_reward_nominal, state_history_nominal)
'''
f = open("results/InvertedPendulum/exp_6/data.txt", "a")


for i in frange(0, 1.02, 0.02):
    episode_reward_n = 0
    Var_n = 0
    terminal_mse = 0
    Var_terminal_mse = 0
    for j in range(n_samples):
        ...
        terminal_mse += np.linalg.norm(state_history[30], axis=0)
        Var_terminal_mse += (np.linalg.norm(state_history[30], axis=0))**2
...
i, x, y, z, a = np.loadtxt("results/InvertedPendulum/exp_6/data.txt", dtype=np.float64, delimiter=',\t',  unpack=True, usecols=(0,1,2,3,4))
i = 100*i
plt.figure(1)
