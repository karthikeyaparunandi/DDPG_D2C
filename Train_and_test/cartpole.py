...
def build_callbacks(env_name):
    checkpoint_weights_filename = '../results/Cartpole/ddpg_' + env_name + '_weights_{step}.h5f'
    log_filename = '../results/Cartpole/exp_7/ddpg_{}_log.json'.format(env_name)
...
# fitting the agent
#agent.fit(env, nb_steps=500000, visualize=True, callbacks=callbacks, verbose=1, gamma=GAMMA, nb_max_episode_steps=STEPS_PER_EPISODE)

# After training is done, we save the final weights.
#agent.save_weights('../results/Cartpole/exp_16/ddpg_{}_weights.h5f'.format(ENV_NAME), overwrite=True)

# -----------------------------------------------------------------------------------------------------------------------------------------
# Testing phase

agent.load_weights('../results/Cartpole/exp_6/ddpg_{}_weights.h5f'.format(ENV_NAME))
