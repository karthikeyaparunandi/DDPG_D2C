...
def build_callbacks(env_name):
    checkpoint_weights_filename = '../results/Swimmer3/exp_10/ddpg_' + env_name + '_weights_{step}.h5f'
    log_filename = '../results/Swimmer3/exp_10/ddpg_{}_log.json'.format(env_name)
...
# fitting the agent
#agent.fit(env, nb_steps=1600000, visualize=False, callbacks=callbacks, verbose=1, gamma=GAMMA, nb_max_episode_steps=STEPS_PER_EPISODE)
...
agent.load_weights('../results/Swimmer3/exp_4/ddpg_{}_weights.h5f'.format(ENV_NAME))
...
history, state_history_nominal, episode_reward_nominal = agent.test(env, nb_episodes=1, visualize=True, action_repetition=1, nb_max_episode_steps=STEPS_PER_EPISODE, \
                                                         initial_state=np.zeros((10,)), std_dev_noise=0, gamma=GAMMA)
