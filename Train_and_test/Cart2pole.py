...
def build_callbacks(env_name):
    ...
    log_filename = 'results/Cart2Pole/exp_1/ddpg_{}_log.json'.format(env_name)
...
random_process = OrnsteinUhlenbeckProcess(size=nb_actions, theta=.15, dt=0.05, mu=0., sigma=.3)
...
# Ctrl + C.
#agent.fit(env, nb_steps=1000000, visualize=False, callbacks=callbacks, verbose=1, gamma=GAMMA, nb_max_episode_steps=60)

# After training is done, we save the final weights.
#agent.save_weights('results/Cart2Pole/exp_1/ddpg_{}_weights.h5f'.format(ENV_NAME), overwrite=True)
agent.load_weights('results/Cart2Pole/exp_1/ddpg_{}_weights.h5f'.format(ENV_NAME))
