import numpy as np
from swimmer6 import agent 
from swimmer6 import env, GAMMA, STEPS_PER_EPISODE
...
path_to_save = "../results/Swimmer6/exp_4/data.txt"
...
for i in frange(0.0, 1.02, 0.05):
    ...
    for j in range(n_samples):
        ...
        history, state_history, episode_reward = agent.test(env, nb_episodes=1, visualize=False, action_repetition=1, \
            nb_max_episode_steps=STEPS_PER_EPISODE, initial_state=np.zeros((1,16)), std_dev_noise=i*u_max, gamma=GAMMA)
        episode_reward_n += episode_reward
        Var_n += (episode_reward)**2
        terminal_mse += np.linalg.norm(state_history[STEPS_PER_EPISODE][0:2] - np.array([0.6, -0.6]), axis=0)
        Var_terminal_mse += (np.linalg.norm(state_history[STEPS_PER_EPISODE][0:2] - np.array([0.6, -0.6]), axis=0))**2
