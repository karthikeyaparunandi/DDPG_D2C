...
const mjtNum ptb_coef = 0.032;
const int step_max = 2000;
const int32_t roll_max = 70000;
...
const mjtNum cal_step = 0.002;
const mjtNum ptb_coef = 0.048;
const int step_max = 120;
const int32_t roll_max = 1000;
...
const mjtNum t_step = 0.5;
const mjtNum cal_step = 0.01;
mjtNum ptb_coef = 0.8;
const int step_max = 6;
...
void check(mjModel* m, mjData* d)
{
	...
	{
		for (h = 0; h < step_max; h++) delta_xc[h][u] = ptb_coef * uabsmax * gaussrand();
	}
...
void setcontrol(mjModel* m, mjData* d)
{
	...
	while (index < step_max)
	{
		for (y = 0; y < m->nq; y++)
		{
			delta_x1[2 * y] = ptb_coef * uabsmax * gaussrand();
			d->qpos[y] = delta_x1[2 * y] + state_nominal[index][2 * y];
		}
		for (y = 0; y < m->nv; y++)
		{
			delta_x1[2 * y + 1] = ptb_coef * uabsmax * gaussrand();
			d->qvel[y] = delta_x1[2 * y + 1] + state_nominal[index][2 * y + 1];
		}
		for (i = 0; i < ctrl_num; i++)
		{
			delta_x1[i + NS] = ptb_coef * uabsmax * gaussrand();
		...
		for (y = 0; y < NS; y++)
		{
			...
			{
				mat[index][y][i] = mat[index][y][i] * (1 - 1 / (trial + 1.0)) + delta_x2[y] * delta_x1[i] / ((trial + 1.0) * ptb_coef * ptb_coef * uabsmax * uabsmax);
			}
		}
	...
	if ((fp = fopen(dfilename, "r")) != NULL)
	{
		...
		if (ctrl_buff[0] == 'C') {
			for (int i = 0; i < step_max * ctrl_num; i++)
			{
				...
				if (fabs(ctrl_nominal[i]) > uabsmax) uabsmax = fabs(ctrl_nominal[i]);
