...
const mjtNum ptb_coef = 0.56;//5.6
...
const int ctrl_num = 1;
const int NS = 4;
const mjtNum K[NS] = { -1.0959, -2.3829, -33.7244, -8.8701 }; //{ -0.0787, - 0.1996, - 13.0807 ,- 3.0313 };
...
const mjtNum slowmotion2 = 0.5; // smaller faster
mjtNum uabsmax = 0;
...
void sysidcheck(mjModel* m, mjData* d)
{
	...
	for (i = 0; i < NS + ctrl_num; i++)
	{
		for (j = 0; j < n_check; j++)
		{
			for (k = 0; k < step_max; k++) delta_xc[j][k][i] = ptb_coef * uabsmax * gaussrand();
		}
	}
	for (i = 0; i < n_check; i++)
	{
...
void errcompare()
{
	...
	for (double ptb = .1; ptb <= 1; ptb += 0.1)
	{
		k = (int)(10 * ptb);
		strcpy(dfilename, dfilepre);
		...
		for (int i = 0; i < n_check; i++)
		{
			...
			{
				delta_u[e] = ptb * uabsmax * gaussrand();
			}
...
int wWinMain(int argc, const char** argv)
{
	...
	if (TOP_FLAG == true)
	{
		...
		#elif defined(PENDULUM)
				state_nominal[0][0] = 2 * PI + ptb_coef * uabsmax * gaussrand();
		#elif defined(CARTPOLE)
				state_nominal[0][2] = -PI + ptb_coef * uabsmax * gaussrand();
		#elif defined(ACROBOT)
				state_nominal[0][0] = 0;
	...
	for (int e = 0; e < step_max * ctrl_num; e++)
	{
		delta_u[e] = ptb_coef * uabsmax * gaussrand();
	}
