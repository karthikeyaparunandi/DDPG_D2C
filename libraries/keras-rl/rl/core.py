...
class Agent(object):
    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
            visualize=False, nb_max_start_steps=0, start_step_policy=None, log_interval=10000, gamma=1.0,
        ...
        try:
            while self.step < nb_steps:
                ...
                # Adding this to include time-step in training process -- karthik
                #observation = np.append(observation, episode_step*0.1)
    ...
    def test(self,
        # Arguments
            ...
            nb_max_episode_steps (integer): Number of steps per episode that the agent performs before
                automatically resetting the environment. Set to `None` if each episode should run
                (potentially indefinitely) until the environment signals a terminal state.
            std_dev_noise: The standard deviation of the additive noise applied on control signal
            initial_state = Initial state of the system - example: [np.pi, 0] or [0, np.pi, 0, 0]
        ...
        for episode in range(nb_episodes):
            ...
            while not done:
                ...
                if self.processor is not None:
                    action = self.processor.process_action(action)
                action = self.perturb(action, std_dev_noise)
                reward = 0.
                accumulated_info = {}
                for _ in range(action_repetition):
                    callbacks.on_action_begin(action)
                    ...
                    if d:
                        ...
                        break
                state_history = np.vstack([state_history, observation])
    ...
    def _on_test_end(self):
        ...
        """
        pass
    def perturb(self, a, std_dev):

        return a+ np.random.normal(np.zeros(np.size(a)), std_dev)
