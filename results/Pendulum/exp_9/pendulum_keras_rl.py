...
def build_callbacks(env_name):
    ...
    log_filename = 'results/Pendulum/exp_10/ddpg_{}_log.json'.format(env_name)
...
random_process = OrnsteinUhlenbeckProcess(size=nb_actions, theta=.15, mu=0., dt=0.1, sigma=.3)
...
f = open("results/Pendulum/exp_8/data.txt", "a")

for i in frange(1.0, 1.02, 0.02):
    episode_reward_n = 0
    Var_n = 0
    terminal_mse = 0
    Var_terminal_mse = 0
...
'''
i, x, y, z, a = np.loadtxt('results/Pendulum/exp_8/data.txt', dtype=np.float64, delimiter=',\t',  unpack=True, usecols=(0,1,2,3,4))
i = 100*i
plt.figure(1)
