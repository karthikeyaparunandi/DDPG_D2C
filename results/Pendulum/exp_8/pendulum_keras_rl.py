...
def build_callbacks(env_name):
    ...
    log_filename = 'results/Pendulum/exp_8/ddpg_{}_log.json'.format(env_name)
...
random_process = OrnsteinUhlenbeckProcess(size=nb_actions, theta=.15, mu=0., dt=0.1, sigma=.25)
...
#agent.save_weights('results/Pendulum/exp_8/ddpg_{}_weights.h5f'.format(ENV_NAME), overwrite=True)
agent.load_weights('results/Pendulum/exp_8/ddpg_{}_weights.h5f'.format(ENV_NAME))

# Finally, evaluate our algorithm for 5 episodes.
history, state_history_nominal, episode_reward_nominal = agent.test(env, nb_episodes=1, visualize=True, action_repetition=1, callbacks=test_callbacks, gamma=GAMMA,nb_max_episode_steps=30, std_dev_noise=0, initial_state=[np.pi, 0.])
print(episode_reward_nominal, state_history_nominal)
u_max = 4.98564

'''
f = open("results/Pendulum/exp_6/data.txt", "a")
...
for i in frange(0.0, 1.0, 0.05):
    ...
    for j in range(n_samples):
        ...
        history, state_history, episode_reward = agent.test(env, nb_episodes=1, visualize=False, action_repetition=1, nb_max_episode_steps=30, initial_state=[np.pi, 0], std_dev_noise=i*u_max, gamma=GAMMA)
...
'''
'''
i, x, y, z, a = np.loadtxt('results/Pendulum/exp_6/data.txt', dtype=np.float64, delimiter=',\t',  unpack=True, usecols=(0,1,2,3,4))
i = 100*i
plt.figure(1)
plt.plot(i,x)
