...
def build_callbacks(env_name):
    checkpoint_weights_filename = 'results/InvertedPendulum/ddpg_' + env_name + '_weights_{step}.h5f'
    log_filename = 'results/InvertedPendulum/exp_3/ddpg_{}_log.json'.format(env_name)
...
# even the metrics!
memory = SequentialMemory(limit=350000, window_length=1)
random_process = OrnsteinUhlenbeckProcess(size=nb_actions, theta=.15, dt=0.1, mu=0., sigma=.2)
...
# Ctrl + C.
agent.fit(env, nb_steps=350000, visualize=False, callbacks=callbacks, verbose=1, gamma=GAMMA, nb_max_episode_steps=30)

# After training is done, we save the final weights.
agent.save_weights('results/InvertedPendulum/exp_3/ddpg_{}_weights.h5f'.format(ENV_NAME), overwrite=True)
#agent.load_weights('results/InvertedPendulum/exp_3/ddpg_{}_weights.h5f'.format(ENV_NAME))
...
'''
f = open("results/InvertedPendulum/exp_1/data.txt", "a")

for i in frange(0.00, .50, 0.05):
    episode_reward_n = 0
    Var_n = 0
    for j in range(n_samples):
        history, state_history, episode_reward = agent.test(env, nb_episodes=1, visualize=False, action_repetition=1, nb_max_episode_steps=30, initial_state=[0, np.pi, 0, 0], std_dev_noise=i*u_max)
...
'''
'''
i, x, y, z = np.loadtxt('results/InvertedPendulum/exp_1/data.txt', dtype=np.float64, delimiter=',\t',  unpack=True, usecols=(0,1,2,3))
i = 100*i
plt.figure(1)
plt.plot(i,x)
