...
def build_callbacks(env_name):
    ...
    log_filename = 'results/InvertedPendulum/exp_4/ddpg_{}_log.json'.format(env_name)
...
random_process = OrnsteinUhlenbeckProcess(size=nb_actions, theta=.15, dt=0.1, mu=0., sigma=.25)
...
#agent.save_weights('results/InvertedPendulum/exp_4/ddpg_{}_weights.h5f'.format(ENV_NAME), overwrite=True)
agent.load_weights('results/InvertedPendulum/exp_4/ddpg_{}_weights.h5f'.format(ENV_NAME))
...
i, x, y, z, a = np.loadtxt('results/InvertedPendulum/exp_3/data.txt', dtype=np.float64, delimiter=',\t',  unpack=True, usecols=(0,1,2,3,4))
i = 100*i
plt.figure(1)
plt.plot(i,x)
