...
def build_callbacks(env_name):
    ...
    log_filename = 'results/Swimmer/exp_3/ddpg_{}_log.json'.format(env_name)
...
# Ctrl + C.
#agent.fit(env, nb_steps=1000000, visualize=False, callbacks=callbacks, verbose=1, gamma=GAMMA, nb_max_episode_steps=800)

# After training is done, we save the final weights.
#agent.save_weights('results/Swimmer/exp_3/ddpg_{}_weights.h5f'.format(ENV_NAME), overwrite=True)
agent.load_weights('results/Swimmer/exp_3/ddpg_{}_weights.h5f'.format(ENV_NAME))
...
history, state_history_nominal, episode_reward_nominal = agent.test(env, nb_episodes=1, visualize=True, action_repetition=1, callbacks=test_callbacks, nb_max_episode_steps=800, \
                                                         initial_state=[0, 0, 0, 0,0,0,0,0], std_dev_noise=0, gamma=GAMMA)
u_max = 20
