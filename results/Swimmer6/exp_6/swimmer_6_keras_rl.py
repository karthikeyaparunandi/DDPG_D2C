...
def build_callbacks(env_name):
    checkpoint_weights_filename = 'results/Swimmer/exp_6/ddpg_' + env_name + '_weights_{step}.h5f'
    log_filename = 'results/Swimmer6/exp_6/ddpg_{}_log.json'.format(env_name)
...
#agent.save_weights('results/Swimmer6/exp_6/ddpg_{}_weights.h5f'.format(ENV_NAME), overwrite=True)
agent.load_weights('results/Swimmer6/exp_6/ddpg_{}_weights_875000.h5f'.format(ENV_NAME))
...
f = open("results/Swimmer6/exp_4/data.txt", "a")



for i in frange(0.0, 1.05, 0.05):
    episode_reward_n = 0
    Var_n = 0
    terminal_mse = 0
    Var_terminal_mse = 0
    for j in range(n_samples):
        ...
        history, state_history, episode_reward = agent.test(env, nb_episodes=1, visualize=False, action_repetition=1, nb_max_episode_steps=1500, initial_state=np.zeros((1,16)), \
                                                    std_dev_noise=i*u_max, gamma=GAMMA)
        episode_reward_n += episode_reward
        Var_n += (episode_reward)**2
        terminal_mse += np.linalg.norm(state_history[800][0:2] - np.array([0.6, -0.6]), axis=0)
        Var_terminal_mse += (np.linalg.norm(state_history[800][0:2]- np.array([0.6, -0.6]), axis=0))**2
...
i, x, y, z, a = np.loadtxt('results/Swimmer6/exp_4/data.txt', dtype=np.float64, delimiter=',\t',  unpack=True, usecols=(0,1,2,3,4))
#print(i)
i = 100*i
plt.figure(1)
