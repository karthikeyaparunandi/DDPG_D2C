...
# even the metrics!
memory = SequentialMemory(limit=2250000, window_length=1)
random_process = OrnsteinUhlenbeckProcess(size=nb_actions, theta=.15, dt=0.006, mu=0., sigma=.25)
...
# Ctrl + C.
#agent.fit(env, nb_steps=2250000, visualize=False, callbacks=callbacks, verbose=1, gamma=GAMMA, nb_max_episode_steps=1500)

# After training is done, we save the final weights.
#agent.save_weights('results/Swimmer6/exp_4/ddpg_{}_weights.h5f'.format(ENV_NAME), overwrite=True)
agent.load_weights('results/Swimmer6/exp_4/ddpg_{}_weights.h5f'.format(ENV_NAME))

# Finally, evaluate our algorithm for 5 episodes.
history, state_history_nominal, episode_reward_nominal = agent.test(env, nb_episodes=1, visualize=False, action_repetition=1, callbacks=test_callbacks, nb_max_episode_steps=1500, \
...
for i in frange(0.0, .12, 0.02):
    ...
    for j in range(n_samples):
        ...
        history, state_history, episode_reward = agent.test(env, nb_episodes=1, visualize=False, action_repetition=1, nb_max_episode_steps=800, initial_state=np.zeros((1,16)), \
