...
control_conv_threshold_percent = .1
sigma_del_u = 0.0005
J_nominal_prev = 0
Q = np.zeros((state_dimemsion, state_dimemsion))#np.eye(state_dimemsion)
Q_terminal = 700*np.array([[3,0,0,0],[0,3,0,0],[0,0,.3,0],[0,0,0,.3]])
R = .0001*np.ones((control_dimension,control_dimension))
goal_state = np.zeros((state_dimemsion,))

init_time = time.time()

D2C = D2C_class(state_dimemsion, control_dimension, env, horizon, lr_0, lr_schedule_k, ctrl_state_freq_ratio=2)
